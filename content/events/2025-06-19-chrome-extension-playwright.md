---
index: 4
title: 'SKG JS Meetup #4: Youtube Chrome Extension & Playwright Coverage'
date: '2025-06-19'
time: '19:00'
location: 'OKThess, 2 Komotinis Street, 54655, Thessaloniki'
rsvpLink: 'https://www.meetup.com/skg-js/events/308275495/'
talks:
  - title: 'Youtube Share Clip - The Journey of Building an Open Source Chrome Browser Extension with 1000+ Users'
    speaker:
      - name: 'Elias Papavasileiou'
        social:
          linkedin: 'https://www.linkedin.com/in/elias-papavasileiou/'
  - title: 'E2E Testing Coverage with Playwright & Istanbul'
    speaker:
      - name: 'John Pourdanis'
        social:
          linkedin: 'https://www.linkedin.com/in/jpourdanis/'
tags: ['chrome-extension', 'open-source', 'testing', 'playwright', 'e2e']
image: 'https://secure.meetupstatic.com/photos/event/b/d/0/d/highres_528348397.jpeg'
---

**Talk 1: Youtube Share Clip - The Journey of Building an Open Source Chrome Browser Extension with 1000+ Users**
Speaker: Elias Papavasileiou, Senior Frontend Tooling Engineer @ Prezi

The behind-the-scenes developer story of Youtube Share Clip:

- Building an open source extension for Chrome
- Technologies used and why
- Promotion and feedback strategies
- Growth to first 100 and 1000 users
- Business side lessons

**Talk 2: E2E Testing Coverage with Playwright & Istanbul**
Speaker: John Pourdanis, Lead Software Engineer in Test @ Vimachem

Integration of Playwright for end-to-end (E2E) testing with Istanbul's code coverage capabilities. Setting up a robust testing environment that automates user interactions and provides detailed coverage reports.
